...
def log(message):
    ...
    if message.from_user.last_name == None:
        bot.send_message(config.logpm, log_message_none, disable_web_page_preview=None, reply_to_message_id=None, reply_markup=None, parse_mode='html', disable_notification=True)
    else:
        bot.send_message(config.logpm, log_message, disable_web_page_preview=None, reply_to_message_id=None, reply_markup=None, parse_mode='html', disable_notification=True) 
...
def start_hello(message):
    ...
    else: bot.send_message(message.chat.id, f'Привет, {message.from_user.first_name} {message.from_user.last_name}!\n\nЭтот бот создает QR-код. Просто нипиши ему что-нибудь. Или отправь стикер, чтобы получить информацию о нем.')
...
def qr_gen(message):
    ...
    try:
        ...
        bot.send_photo(message.chat.id, qrpic, reply_to_message_id=message.message_id)
